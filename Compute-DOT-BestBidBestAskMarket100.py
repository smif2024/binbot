...
def compute_arbitrage(logger):
    ...
    if profit_or_loss > 0:
        ...
        profit_filename = f"logs/compute/profit-DOT-100-{datetime.now().strftime('%d%m%Y%H')}.csv"  # Update filename pattern to hourly
...
log_filename = f"logs/compute/compute-DOT-100-{datetime.now().strftime('%d%m%Y%H')}.log"
...
while True:
    ...
    new_log_filename = f"logs/compute/compute-DOT-100-{datetime.now().strftime('%d%m%Y%H')}.log"
