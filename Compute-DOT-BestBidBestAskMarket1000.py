...
def compute_arbitrage(logger):
    ...
    dot_brl_bid, dot_brl_ask = get_best_prices('DOTBRL', logger)     # BUY DOT with BRL
    dot_usdt_bid, dot_usdt_ask = get_best_prices('DOTUSDT', logger)  # SELL DOT for USDT
    ...
    logger.info(f"DOTBRL: Best Bid = {dot_brl_bid}, Best Ask = {dot_brl_ask}")
    logger.info(f"DOTUSDT: Best Bid = {dot_usdt_bid}, Best Ask = {dot_usdt_ask}\n")
    ...
    # Step 3: Calculate the amount of DOT we can buy with BRL at the best ask price
    dot_bought = brl_received / dot_brl_ask

    # Step 4: Calculate the amount of USDT obtained by selling DOT at the best bid price
    usdt_received = dot_bought * dot_usdt_bid
    ...
    logger.info(f"DOT bought with BRL at best ask price: {dot_bought:.6f} DOT")
    logger.info(f"USDT received after selling DOT at best bid price: {usdt_received:.2f} USDT")
    ...
    if profit_or_loss > 0:
        ...
        profit_filename = f"logs/compute/profit-DOT-1000-{datetime.now().strftime('%d%m%Y%H')}.csv"  # Update filename pattern to hourly
        with open(profit_filename, mode='a', newline='', encoding='utf-8') as profit_file:
            writer = csv.writer(profit_file)
            writer.writerow([datetime.now().strftime('%Y-%m-%d %H:%M:%S'), eth_bought, brl_received, dot_bought, usdt_received, profit_or_loss])
...
log_filename = f"logs/compute/compute-DOT-1000-{datetime.now().strftime('%d%m%Y%H')}.log"
...
while True:
    ...
    new_log_filename = f"logs/compute/compute-DOT-1000-{datetime.now().strftime('%d%m%Y%H')}.log"
